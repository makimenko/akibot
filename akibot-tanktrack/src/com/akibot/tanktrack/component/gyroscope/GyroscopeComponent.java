...
public class GyroscopeComponent extends DefaultComponent {
	...
	private GyroscopeOffsetConfiguration gyroscopeConfiguration;
	...
	public GyroscopeComponent(int busNumber, int deviceAddress) throws IOException, InterruptedException {
	...
	private void selfTest(byte gain, byte cra) throws IOException, InterruptedException {
		...
		i2cDevice.write(configurationRegisterA, cra);
		i2cDevice.write(configurationRegisterB, gain);
		modeContinuousMeasurement();
		Thread.sleep(10);

		System.out.println("-");
		for (int i = 0; i <= 7; i++) {
			int x = readShort(dataOutputMSBRegisterX, i2cDevice);
			int y = readShort(dataOutputMSBRegisterY, i2cDevice);
			int z = readShort(dataOutputMSBRegisterZ, i2cDevice);
			System.out.println("Test: " + x + ", " + y + ", " + z);
			Thread.sleep(80);
		}
		// TODO: Still unclear how to validate output!
		i2cDevice.write(configurationRegisterA, cra_8samples_15Hz_normal); // Exit self test mode
	}

	public void modeIdle() throws IOException, InterruptedException {
		i2cDevice.write(modeRegister, idle_mode);
		Thread.sleep(7);
	}

	public void modeSingleMeasurement() throws IOException, InterruptedException {
		i2cDevice.write(modeRegister, single_measurement_mode);
		Thread.sleep(7);
	}

	public void modeContinuousMeasurement() throws IOException, InterruptedException {
		i2cDevice.write(modeRegister, continuous_measurement_mode);
		Thread.sleep(7);
	}
	...
	private void onGyroscopeConfigurationRequest(GyroscopeConfigurationRequest gyroscopeConfigurationRequest) {
		gyroscopeConfiguration = gyroscopeConfigurationRequest.getGyroscopeOffsetConfiguration();
	...
	private void onGyroscopeValueRequest(GyroscopeValueRequest gyroscopeValueRequest) throws IOException, FailedToSendMessageException {
		...
		double x = readShort(dataOutputMSBRegisterX, i2cDevice);
		double y = readShort(dataOutputMSBRegisterY, i2cDevice);
		double z = readShort(dataOutputMSBRegisterZ, i2cDevice);
	...
	public void loadConfiguration() throws FailedToConfigureException {
		try {
			...
			this.gyroscopeConfiguration = (GyroscopeOffsetConfiguration) getConfigurationResponse.getValue();
		} catch (Exception e) {
	...
	public void startComponent() throws FailedToStartException {
		try {
			...
			i2cDevice = bus.getDevice(deviceAddress);

			modeIdle();
			// selfTest(gain_0_88, cra_8samples_15Hz_normal);
			// selfTest(gain_8_1, cra_8samples_15Hz_normal);

			i2cDevice.write(configurationRegisterA, cra_8samples_15Hz_normal);
			i2cDevice.write(configurationRegisterB, gain_1_3_default);
			modeContinuousMeasurement();
