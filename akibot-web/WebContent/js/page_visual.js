...
var axisHelper;
var currentCamera = "1";
var gridGroup, grid, cellObjectTemplate;
var cellSizeMm;
var cellScale = 0.1;
...
function onWsMessage(message) {
	...
	console.log("onWsMessage: jsonDataStr: " + jsonDataStr);
...
function addLocationAreaGrid(node, parentSceneNode) {
	...
	if (transformation == null) {
		...
		transformation.position.x += offsetMm.x;
		transformation.position.y += offsetMm.y;
	}
	...
	cellSizeMm = node.geometry.gridConfiguration.cellSizeMm;
	var geometry = new THREE.BoxGeometry(cellSizeMm * cellScale - 0.1, cellSizeMm * cellScale - 0.1, 0.5);
...
function addGridCell(node, x, y, value) {
	...
	cellObject.position.x = x * cellSizeMm * cellScale + (cellSizeMm * cellScale / 2);
	cellObject.position.y = y * cellSizeMm * cellScale + (cellSizeMm * cellScale / 2);
...
function applyTransformation(object3d, transformation) {
	if (transformation != null) {
		...
		if (transformation.position != null) {
			object3d.position
					.set(transformation.position.x * cellScale, transformation.position.y * cellScale, transformation.position.z * cellScale);
		}
		// Rotation:
